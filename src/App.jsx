...
export default function YamsUltimateLegacy() {
  ...
  const updateScore=(player,category,value, event)=>{
    ...
    if(categories.find(c=>c.id===category)?.upper && value !== '') {
      ...
      if(allUpperFilled && currentUpperSum < 63) {
        setShowBonusLostNotif(player);
        setConfetti('sad');
        setTimeout(()=>{setShowBonusLostNotif(null);setConfetti(null);},4500);
  ...
  const resetGame = (forcedLoserName = null) => { 
      ...
      setScores({}); setLastPlayerToPlay(null); setLastModifiedCell(null); setShowEndGameModal(false); setMoveLog([]); setActiveChaosCard(null); setShowStudioModal(false); setSuddenDeathWinner(null); setSuddenDeathPlayers([]); setShowSuddenDeath(false);
  ...
  useEffect(()=>{if(isGameComplete()&&!showEndGameModal&&!showSuddenDeath){
    ...
    if(winners.length > 1 && players.length > 1) {
      ...
      setShowVictoryAnimation(true);setConfetti('gold');setTimeout(()=>{setShowVictoryAnimation(false);setShowEndGameModal(true);setConfetti(null);},2000);
    }
...
`}</style>
      ...
      {showBonusLostNotif&&(
        <div className="fixed top-20 right-4 z-50 slide-in-right">
          <div className="relative overflow-hidden px-6 py-5 rounded-2xl shadow-2xl backdrop-blur-xl border-2 max-w-sm bg-gradient-to-r from-red-700 to-rose-700 border-red-400 shake-animation">
            <div className="absolute inset-0" style={{animation:'shimmer 2s infinite',backgroundSize:'200% 100%',backgroundImage:'linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent)'}}></div>
            <div className="flex items-center gap-4 relative z-10">
              <span className="text-5xl" style={{animation:'sad-pulse 1.5s ease-in-out infinite'}}>💔</span>
              <div className="text-white">
                <div className="text-xs font-bold uppercase tracking-widest opacity-80">😢 Dommage !</div>
                <div className="font-black text-xl">Bonus Raté !</div>
                <div className="text-sm opacity-90">{showBonusLostNotif} ne peut plus obtenir le bonus de 35 pts</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className={'bg-gradient-to-br '+T.card+' backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl '+T.glow+' p-4 sm:p-6'}>
          ...
          {showSettings&&<div className="mt-6 pt-6 border-t border-white/10 slide-down"><h3 className="text-xs font-bold text-gray-400 mb-3 uppercase tracking-wider flex items-center gap-2"><Palette size={14}/> Thème</h3><div className="grid grid-cols-2 sm:grid-cols-4 gap-3">{Object.keys(THEMES_CONFIG).map(k=>{const td=THEMES_CONFIG[k];return <button key={k} onClick={()=>setTheme(k)} className={'relative overflow-hidden px-4 py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 '+(theme===k?'ring-2 ring-white scale-105':'hover:scale-105')} style={{background:'linear-gradient(135deg,'+td.primary+','+td.secondary+')',color:'#fff'}}>{theme===k? <Check size={16}/> : td.icon}<span>{td.name}</span></button>;})}</div>
          ...
          <div className="flex gap-2 mt-4 flex-wrap">
            ...
            <button onClick={()=>setCurrentTab('rules')} className={'flex-1 min-w-[80px] py-3 rounded-xl font-bold transition-all duration-300 hover-float active:scale-95 '+(currentTab==='rules'?'text-white shadow-xl scale-[1.02] '+T.glow:'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10 hover:text-white')} style={currentTab==='rules'?{background:'linear-gradient(135deg,'+T.primary+','+T.secondary+')'}:{}}>🎲 Règles & Aide</button>
            <button onClick={()=>setCurrentTab('trophies')} className={'flex-1 min-w-[80px] py-3 rounded-xl font-bold transition-all duration-300 hover-float active:scale-95 '+(currentTab==='trophies'?'text-white shadow-xl scale-[1.02] '+T.glow:'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10 hover:text-white')} style={currentTab==='trophies'?{background:'linear-gradient(135deg,'+T.primary+','+T.secondary+')'}:{}}>🏆 Carrière</button>
            ...
            <button onClick={()=>setCurrentTab('stats')} className={'flex-1 min-w-[80px] py-3 rounded-xl font-bold transition-all duration-300 hover-float active:scale-95 '+(currentTab==='stats'?'text-white shadow-xl scale-[1.02] '+T.glow:'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10 hover:text-white')} style={currentTab==='stats'?{background:'linear-gradient(135deg,'+T.primary+','+T.secondary+')'}:{}}>📊 Stats</button>
            <button onClick={()=>setCurrentTab('gages')} className={'flex-1 min-w-[80px] py-3 rounded-xl font-bold transition-all duration-300 hover-float active:scale-95 '+(currentTab==='gages'?'text-white shadow-xl scale-[1.02] '+T.glow:'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10 hover:text-white')} style={currentTab==='gages'?{background:'linear-gradient(135deg,'+T.primary+','+T.secondary+')'}:{}}>😈 Gages</button>
          </div>
        </div>
            ...
            <div className="space-y-6 tab-enter">
                ...
                <div className={'bg-gradient-to-br '+T.card+' backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl '+T.glow+' p-6'}>
                    ...
                    {(()=>{
                        ...
                        return (
                        <div className="mb-6">
                            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">🎲 Distribution des Yams réalisés ({totalYams} au total)</h3>
                            <div className="grid grid-cols-6 gap-2 mb-4">
                                {[1,2,3,4,5,6].map(val => (
                                    <div key={val} className="text-center p-3 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-300 hover:scale-105" style={{animation:`fade-in-scale 0.3s ease-out ${val*0.05}s backwards`}}>
                                        <div className="text-3xl mb-1">{['','⚀','⚁','⚂','⚃','⚄','⚅'][val]}</div>
                                        <div className="text-white font-black text-xl">{dist[val]||0}</div>
                                        <div className="text-[10px] text-gray-500 uppercase font-bold">yams</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {(()=>{
                            ...
                            (g.players||g.results||[]).forEach(p => {
                                ...
                                upperCatConfig.forEach(cat => {
                                    ...
                                    if(val !== undefined && parseInt(val) === cat.max) {
                                        hiddenYamsData[p.name].total++;
                                        hiddenYamsData[p.name].details[cat.id] = (hiddenYamsData[p.name].details[cat.id]||0) + 1;
                                    }
                                });
                            });
                        });
                        const globalDetails = {};
                        upperCatConfig.forEach(cat => {
                            globalDetails[cat.id] = Object.values(hiddenYamsData).reduce((s,p) => s + (p.details[cat.id]||0), 0);
                        });
                        const globalTotal = Object.values(globalDetails).reduce((s,v)=>s+v,0);
                        const sortedPlayers = Object.entries(hiddenYamsData).sort((a,b) => b[1].total - a[1].total);
                        
                        return (
                        <div>
                            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">🫣 Yams Cachés <span className="text-[10px] text-gray-500 normal-case font-normal ml-2">(score max sur un chiffre : 5 au As, 10 au Deux, ... 30 au Six)</span></h3>
                            
                            {/* Global view */}
                            <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-400/30 rounded-2xl p-4 mb-4">
                                <div className="flex items-center justify-between mb-3">
                                    <span className="text-purple-300 font-bold text-sm uppercase tracking-wider">🌍 Global</span>
                                    <span className="text-white font-black text-2xl">{globalTotal} <span className="text-sm text-gray-400 font-normal">yams cachés</span></span>
                                </div>
                                <div className="grid grid-cols-6 gap-2">
                                    {upperCatConfig.map(cat => (
                                        <div key={cat.id} className="text-center p-2 rounded-xl bg-black/20 border border-white/5">
                                            <div className="text-xl">{cat.icon}</div>
                                            <div className="text-white font-black">{globalDetails[cat.id]||0}</div>
                                            <div className="text-[9px] text-gray-500">{cat.max} pts</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                {sortedPlayers.map(([name, data], idx) => (
                                    <div key={name} className="bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 transition-all duration-300" style={{animation:`fade-in-scale 0.3s ease-out ${idx*0.05}s backwards`}}>
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-white font-bold flex items-center gap-2"><span className="text-lg">{playerAvatars[name]||'👤'}</span> {name}</span>
                                            <span className="text-purple-400 font-black">{data.total} yams cachés</span>
                                        </div>
                                        <div className="flex gap-2">
                                            {upperCatConfig.map(cat => (
                                                <div key={cat.id} className={`flex-1 text-center py-1 rounded-lg text-xs font-bold ${(data.details[cat.id]||0) > 0 ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30' : 'bg-black/20 text-gray-600 border border-white/5'}`}>
                                                    {cat.icon} {data.details[cat.id]||0}
                                                </div>
                                            ))}
                                        </div>
                                    </div>
